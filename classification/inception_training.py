    ...
    for epoch in range(NUM_EPOCHS):  # loop over the dataset multiple times
        ...
        for j, val_data in enumerate(valloader, 0):
            with torch.no_grad():
                ...
                # forward
                outputs_val, axu1_val_outputs, aux2_val_outputs = net(inputs_val)
                loss1_val = criterion(outputs_val, labels_val)  # error value
                loss2_val = criterion(axu1_val_outputs, labels_val)  # error value
                loss3_val = criterion(axu2_val_outputs, labels_val)  # error value
                loss_val = loss1_val + 0.3*(loss2_val+loss3_val)
...
if __name__ == '__main__':
    ...
    PATH='experiments/models/inception/inception_v1.pth'  # Card 3
